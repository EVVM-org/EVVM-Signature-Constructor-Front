...
export const PublicStakingSignatureConstructor = () => {
  ...
  const { signPublicStaking } = useEVVMSignatureBuilder();
  const [isStaking, setIsStaking] = React.useState(true);
  const [priority, setPriority] = React.useState("low");
  const [payDataInfo, setPayDataInfo] = React.useState<PayData | null>(null);
  const [publicStakingDataInfo, setPublicStakingDataInfo] =
    React.useState<PublicStakingData | null>(null);
  ...
  return (
    <div className="flex flex-1 flex-col justify-center items-center">
      ...
      {/* Results Section */}
      {payDataInfo && publicStakingDataInfo && (
        <div style={{ marginTop: "2rem" }}>
          ...
          {showData && (
            ...
            >
              ...
              <h3>Staking Data:</h3>
              {Object.entries(publicStakingDataInfo).map(([key, value]) => (
                <div key={key} style={{ marginBottom: "0.5rem" }}>
                  <strong>{key}:</strong> {value}
                  <button
                    onClick={() => navigator.clipboard.writeText(value)}
            ...
            <button
              onClick={() =>
                navigator.clipboard.writeText(
                  JSON.stringify(publicStakingDataInfo, null, 2)
                )
              }
              style={{
                padding: "0.5rem",
